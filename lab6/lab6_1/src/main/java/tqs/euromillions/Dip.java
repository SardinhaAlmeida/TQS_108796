        ...
        */
        ...
        if (NUMBERS_REQUIRED == arrayOfNumbers.length && STARS_REQUIRED == arrayOfStarts.length) {
            ...
            for (int number : arrayOfNumbers) {
                if (number > NUMBERS_RANGE_MAX){
                    throw new IllegalArgumentException("number outside the expected range!");
                }
            }
            
            for (int star : arrayOfStarts) {
                if (star > STARS_RANGE_MAX){
                    throw new IllegalArgumentException("star outside the expected range!");
                }
            }
